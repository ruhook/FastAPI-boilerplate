...
def _install_fake_redis(application: Any, redis: FakeVerificationRedis) -> None:
    async def override_redis() -> FakeVerificationRedis:
        return redis

    application.dependency_overrides[async_get_redis] = override_redis


async def test_local_virtual_admin_login_succeeds_repeatedly(
    client: AsyncClient,
...
async def test_verification_check_consumes_valid_code_without_central_counter() -> None:
    redis = FakeVerificationRedis()
    ...
    assert cache_key not in redis.storage
