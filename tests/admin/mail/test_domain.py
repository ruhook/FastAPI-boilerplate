...
async def test_superadmin_can_manage_mail_templates_signatures_assets_and_send_tasks(
    ...
    assert task["account_email"] == "templates@example.com"
    assert task["template_id"] == template_id
    assert task["template_name"] == "UK 测试题通知"
    assert task["signature_id"] == signature_id
    assert task["signature_name"] == "招聘标准签名"
    assert task["status"] == "pending"
    assert task["status_cn_name"] == "待处理"
    ...
    assert task_list[0]["id"] == task["id"]

    resend_response = await client.post(
        f"/api/v1/mail/tasks/{task['id']}/resend",
        headers=admin_auth_headers,
    )
    assert resend_response.status_code == 200, resend_response.text
    resent_task = resend_response.json()
    assert resent_task["id"] != task["id"]
    assert resent_task["status"] == "pending"
    assert resent_task["data"]["resend_from_task_id"] == task["id"]
    ...
    async with local_session() as session:
        ...
        for expected in (
            AdminAuditLogActionType.MAIL_ACCOUNT_CREATED.value,
            AdminAuditLogActionType.MAIL_TEMPLATE_CATEGORY_CREATED.value,
            AdminAuditLogActionType.MAIL_TEMPLATE_CREATED.value,
            AdminAuditLogActionType.MAIL_TEMPLATE_UPDATED.value,
            AdminAuditLogActionType.MAIL_SIGNATURE_CREATED.value,
            AdminAuditLogActionType.MAIL_SIGNATURE_UPDATED.value,
            AdminAuditLogActionType.MAIL_TEMPLATE_DELETED.value,
            AdminAuditLogActionType.MAIL_SIGNATURE_DELETED.value,
            AdminAuditLogActionType.MAIL_TEMPLATE_CATEGORY_DELETED.value,
        ):
