...
async def test_superadmin_can_create_update_and_delete_role(
    ...
    list_response = await client.get("/api/v1/roles", headers=admin_auth_headers)
    assert list_response.status_code == 200, list_response.text
    list_data = list_response.json()
    assert len(list_data) == 1
    assert list_data[0]["permissions"] == ["管理账号", "管理角色"]
    ...
    assert updated_role["permissions"] == ["管理账号"]

    delete_response = await client.delete(f"/api/v1/roles/{role_id}", headers=admin_auth_headers)
    assert delete_response.status_code == 200, delete_response.text
    assert delete_response.json()["message"] == "Role deleted."

    list_after_delete_response = await client.get("/api/v1/roles", headers=admin_auth_headers)
