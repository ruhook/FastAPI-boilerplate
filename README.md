...
Mail-task events use a transactional database outbox: creating the task and recording its event now commit together. The event process dispatches leased outbox rows to Redis with a stable event id, retries publication with bounded backoff, reclaims stale Redis pending messages, recovers abandoned rendering tasks, and ACKs only after every handler succeeds. An abandoned SMTP sending state becomes `delivery_unknown` for operator review instead of being blindly retried.
