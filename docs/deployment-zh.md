...
`/api/v1/health` 只检查进程存活；`/api/v1/ready` 会并发检查 lifespan 初始化、MySQL 和 Redis，任何一项失败或超过 `HEALTH_CHECK_TIMEOUT_SECONDS` 都返回 503。负载均衡器应使用 `/ready` 决定是否接流量。
...
cd src && uv run --frozen alembic upgrade head
cd ..
...
`20260710_000042` 会新增 `event_outbox`。邮件任务与待发布事件在同一个数据库事务中提交，`event_consumer.py` 同时负责 outbox 派发、Redis 消费和过期邮件处理租约恢复；因此生产环境必须持续托管这个进程。Redis 发布失败会按退避重试，超过上限的行保留为 `failed`，需要运维查询和人工重试。过期 `rendering` 会重新排队；SMTP `sending` 中断会进入 `delivery_unknown`，确认收件情况前不要自动重发。
