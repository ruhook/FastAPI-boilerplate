...
@router.get("", response_model=list[RoleRead], dependencies=[Depends(require_admin_permission("管理角色"))])
async def read_roles(db: Annotated[AsyncSession, Depends(async_get_db)]) -> list[dict[str, Any]]:
    return await list_roles(db)


@router.post("", response_model=RoleRead, status_code=201, dependencies=[Depends(require_admin_permission("管理角色"))])
...
@router.get("/{role_id}", response_model=RoleRead, dependencies=[Depends(require_admin_permission("管理角色"))])
...
@router.patch("/{role_id}", response_model=RoleRead, dependencies=[Depends(require_admin_permission("管理角色"))])
...
@router.delete("/{role_id}", dependencies=[Depends(require_admin_permission("管理角色"))])
