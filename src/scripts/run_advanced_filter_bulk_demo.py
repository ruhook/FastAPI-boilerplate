...
from .seed_timesheet_demo_flow import (
    DEMO_TIMESHEET_LANGUAGES,
    DEMO_TIMESHEET_ROLES,
    DEMO_TIMESHEET_WORK_TYPES,
...
async def ensure_candidate_resume_asset(
    ...
    *,
    ...
    content = (
        f"Advanced filter bulk demo resume for {email}\n"
        "Generated by run_advanced_filter_bulk_demo.py\n"
    ).encode("utf-8")
...
def build_progress_candidate_definition(*, run_tag: str, index: int) -> dict[str, Any]:
    ...
    nationalities = ["Brazilian", "Singaporean", "Japanese", "Mexican"]
...
def build_progress_application_items(
    *,
    ...
    return [
        ...
        {"field_key": CandidateFieldKey.AGE_RANGE.value, "value": definition["age_range"], "display_value": definition["age_range"]},
        {
        ...
        batch_templates = [
            {
                ...
                "human_efficiency_minutes": Decimal("18"),
                "team_leader_index": 1,
            ...
            },
            {
                ...
                "human_efficiency_minutes": Decimal("16"),
                "team_leader_index": 2,
            ...
            },
            {
                ...
                "human_efficiency_minutes": Decimal("20"),
                "team_leader_index": 3,
            ...
            },
            {
                ...
                "human_efficiency_minutes": Decimal("24"),
                "team_leader_index": 2,
            ...
            },
            {
                ...
                "human_efficiency_minutes": Decimal("15"),
                "team_leader_index": 3,
        ...
        for batch_index in range(batch_count):
            ...
            for entry_index in range(batch_size):
                ...
                customer_duration_hours = (
                    template["human_efficiency_minutes"] * output_quantity / Decimal("60")
                ...
                records_meta.append(
                    {
                        ...
                        "human_efficiency_minutes": float(template["human_efficiency_minutes"]),
            ...
            payload = ProjectTimesheetBatchCreateRequest(
                ...
                language=str(template["language"]),
                project_link=str(template["project_link"]),
                human_efficiency_minutes=Decimal(template["human_efficiency_minutes"]),
                team_leader_user_id=int(leader_contract.user_id),
                entries=entries,
            )
...
def build_candidate_query(*, run_tag: str) -> dict[str, Any]:
    return {
        "combinator": "and",
        "rules": [
            {"field": "nationality", "operator": "contains", "value": "Brazilian"},
...
def expected_candidate_count(rows: list[dict[str, Any]], *, run_tag: str) -> int:
    return len(
        [
            ...
            for row in rows
            if run_tag in row["full_name"]
            and "Brazilian" in row["nationality"]
            and row["education_status"] == "bachelor_completed"
...
async def main() -> None:
    ...
    report_path = Path(args.report_path) if args.report_path else report_dir / f"advanced-filter-bulk-{make_slug(run_tag)}.json"
    ...
    if actual_progress_screening != expected_progress_screening:
        raise RuntimeError(
            f"Progress screening advanced filter mismatch: expected {expected_progress_screening}, got {actual_progress_screening}"
        )
    if actual_progress_contract != expected_progress_contract:
        raise RuntimeError(
            f"Progress contract advanced filter mismatch: expected {expected_progress_contract}, got {actual_progress_contract}"
        )
    if actual_candidates != expected_candidates:
        raise RuntimeError(
            f"Talent advanced filter mismatch: expected {expected_candidates}, got {actual_candidates}"
        )
    if len(actual_timesheet_rows) != len(expected_timesheet_filtered_rows):
        raise RuntimeError(
            f"Timesheet advanced filter mismatch: expected {len(expected_timesheet_filtered_rows)}, got {len(actual_timesheet_rows)}"
        )
    if actual_dashboard != expected_dashboard:
        raise RuntimeError(
            f"Timesheet dashboard mismatch: expected {expected_dashboard}, got {actual_dashboard}"
        )
    ...
    print_detail(f"progress seeded={len(progress_rows)} screening_match={actual_progress_screening} contract_match={actual_progress_contract}")
